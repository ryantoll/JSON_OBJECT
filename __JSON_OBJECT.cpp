...
string JSON_OBJECT::JSONObjectToString() {
	...
	else if (IsArray()) { return JSONArrayToString(*valueArray); }							// Print array if it is an array
	else if (data.has_value()) {
		for (auto prop : *data) {															// Print all properties if it's an object
			output += "\"" + prop.first + "\":";
			output += prop.second.JSONObjectToString();
			output += ",";																	// Deliniate elements with commas
		}
		output.erase(output.size() - 1);													// Erase the trailing comma
	}
	else { return "null"; }																	// Print "null" to represent null value	

	output += "}";																			// End with close-curly-brace
	return output;
}

// Private helper function. Seperates out logic for building array strings.
string JSON_OBJECT::JSONArrayToString(vector<JSON_OBJECT>& jsonArray) const {
	string output{ "[" };																		// Open-square-bracket starts array
	for (auto element : jsonArray) { output += element.JSONObjectToString(); output += ","; }	// Iterate over elements, add each one, and deliniate with commas
	output.erase(output.size() - 1);															// Erase the trailing comma
...
JSON_OBJECT& JSON_OBJECT::StringToJSON(string& input) {
	...
	if (ClearEnclosingChars('[', ']', input)) { ParseJSONArrayString(input); return *this; }			// Build JSON array
	...
	while (end != string::npos && end < input.size()) {
		...
		key = input.substr(0, end);
		ClearEnclosingChars('"', '"', key);
		input.erase(0, end + 1);							// Erase read portion as well as colon

		end = input.find_first_of(",");
		val = input.substr(0, end);
		tempJSON.StringToJSON(val);
		AddProperty(key, tempJSON);							// Recursively build
		input.erase(0, end + 1);							// Erase read portion as well as comma
	}
	return *this;											// Return reference to self
}

// Private helper function. Separates logic for parsing arrays.
void JSON_OBJECT::ParseJSONArrayString(string& input) {
	string segment{ "" };
	JSON_OBJECT tempJSON;
	...
	while (input.size() > 0) {
		segment = input.substr(0, n + 1);
		tempJSON.StringToJSON(segment);
		push_back(tempJSON);
		n = input.find_first_of(delimiter);
		input.erase(0, n + 2);
	}
}
